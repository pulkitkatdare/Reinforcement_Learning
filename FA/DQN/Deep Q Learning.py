...
def deep_q_learning(sess,
    ...
    for i in range(replay_memory_init_size):
        temp_memory = []
        temp_memory.append(states)
        A = policy(sess,state,epsilons[total_t])
        action  = np.random.choice(range(len(A)),p = A)
        observation, reward, done, info = env.step(action)
        state_next = state_processor.process(sess, observation)
        new_state = np.zeros((84,84,4))
        new_state[:,:,0:3] = state[:,:,1:]
        new_state[:,:,3] = state_next
        replay_memory.append(Transition(state, action, reward, new_state, done))
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            loss = 0 
            s = np.zeros((batch_size,84,84,4))
            a = np.zeros(batch_size)
            y = np.zeros(batch_size)
            for n in range(batch_size):
                index = np.random.randint(len(replay_memory))
                ss,aa,rr,ss_dash,done_dash = replay_memory[index]
                s[n,:,:,:] = ss
                a[n] = aa
                y[n] = rr + discount_factor*max(target_estimator.predict(sess, np.expand_dims(ss_dash, 0))[0])
            loss = q_estimator.update(sess,s,a,y)
